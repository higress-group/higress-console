...
import { useNavigate, useParams, useSelector, useDispatch } from 'ice';
import { 
  getMCPConfig, 
  createMCPConfig, 
  updateMCPConfig 
...
const EditMCPPage: React.FC<EditMCPPageProps> = ({ editing = false }) => {
  ...
  const mcpConfig = useSelector((state) => state.mcp.configurations.find(c => c.id === id));
  ...
  const fetchConfig = async (configId: string) => {
    try {
      setLoading(true);
      const response = await getMCPConfig(configId);
  ...
  return (
    <MCPLLayout>
      <Card
        title={
          <Space>
            <Button type="link" icon={<LeftOutlined />} onClick={() => navigate('/mcp')}>
              Back
            </Button>
          ...
          <Tabs defaultActiveKey="1" style={{ marginTop: 16 }}>
            ...
            <Tabs.TabPane tab="Advanced" key="2" disabled={!isEditing}>
              <Card title="Performance Monitoring">
                <Form.Item label="Success Rate Threshold" name="successRateThreshold" rules={[{ type: 'number', min: 0, max: 100 }]}>
                  <Input suffix="%" />
                </Form.Item>
