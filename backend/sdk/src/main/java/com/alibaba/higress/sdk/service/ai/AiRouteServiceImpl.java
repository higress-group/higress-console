...
public class AiRouteServiceImpl implements AiRouteService {
    ...
    public AiRoute query(String routeName) {
        V1ConfigMap configMap = readAiRouteConfigMap(routeName);
        return Optional.ofNullable(configMap).map(this::configMap2AiRoute).orElse(null);
    }

    private V1ConfigMap readAiRouteConfigMap(String routeName) {
    ...
    public AiRoute update(AiRoute route) {
        fillDefaultValues(route);
        V1ConfigMap previousConfigMap = readAiRouteConfigMap(route.getName());
        ...
        try {
            ...
            rollbackAiRouteUpdate(route.getName(), previousConfigMap, updatedConfigMap);
            throw e;
        }
    ...
    /**
     * Update the ConfigMap first. If updating related resources fails, try to restore the previous ConfigMap and
     * resources. This is a best-effort rollback, not an atomic transaction, but it should be enough for user-driven
     * console operations.
     */
    private void rollbackAiRouteUpdate(String routeName, V1ConfigMap previousConfigMap, V1ConfigMap updatedConfigMap) {
        V1ObjectMeta previousMetadata = previousConfigMap == null ? null : previousConfigMap.getMetadata();
        V1ObjectMeta updatedMetadata = updatedConfigMap == null ? null : updatedConfigMap.getMetadata();
        if (previousMetadata == null || updatedMetadata == null) {
            log.error("AI route operation result: routeName={}, resource=configMap, status=failed, "
                + "reason=metadataMissing", routeName);
            return;
        }
        previousMetadata.setResourceVersion(updatedMetadata.getResourceVersion());
        if (!safeRun(routeName, "configMap", () -> kubernetesClientService.replaceConfigMap(previousConfigMap))) {
            return;
        }

        AiRoute previousRoute = configMap2AiRoute(previousConfigMap);
        if (previousRoute == null) {
            log.warn("AI route operation result: routeName={}, resource=runtime, status=skipped, "
                + "reason=configMapConversionFailed", routeName);
            return;
        }

        safeRun(routeName, "primaryRuntime", () -> writeAiRouteResources(previousRoute));
        safeRun(routeName, "fallbackRuntime", () -> writeAiRouteFallbackResources(previousRoute));
    }

    private boolean safeRun(String routeName, String resource, FailableRunnable<Exception> action) {
        try {
            action.run();
            log.info("AI route operation result: routeName={}, resource={}, status=succeeded", routeName, resource);
            return true;
        } catch (Exception e) {
            log.error("AI route operation result: routeName={}, resource={}, status=failed", routeName, resource, e);
            return false;
        }
    }
