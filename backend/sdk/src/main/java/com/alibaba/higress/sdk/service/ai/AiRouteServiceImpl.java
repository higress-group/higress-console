...
public class AiRouteServiceImpl implements AiRouteService {
    ...
    private void validateResourceVersion(AiRoute route) {
        String configMapName = kubernetesModelConverter.aiRouteName2ConfigMapName(route.getName());
        V1ConfigMap currentConfigMap;
        try {
            currentConfigMap = kubernetesClientService.readConfigMap(configMapName);
        } catch (ApiException e) {
            throw new BusinessException(
                "Error occurs when reading the ConfigMap generated by AI route: " + route.getName(), e);
        }
        if (currentConfigMap == null || currentConfigMap.getMetadata() == null) {
            throw new BusinessException("No ConfigMap is found for AI route: " + route.getName());
        }
        if (!StringUtils.equals(route.getVersion(), currentConfigMap.getMetadata().getResourceVersion())) {
            throw new ResourceConflictException();
        }
    }
